...
function ForgotPassword() {
  ...
  const [cooldown, setCooldown] = useState(() => {
    const until = sessionStorage.getItem('forgotpw_cooldown')
    if (!until) return 0
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setCooldown((prev) => {
        const next = prev - 1
        if (next <= 0) {
          sessionStorage.removeItem('forgotpw_cooldown')
          return 0
        }
        sessionStorage.setItem('forgotpw_cooldown', String(Date.now() + next * 1000))
        return next
      })
    }, 1000)
  ...
  async function handleSendCode(e: FormEvent) {
    ...
    if (email === lastSentEmail.current) {
      setStep('reset')
      return
    }
    setLoading(true)
    ...
    if (resetErr) {
      ...
      if (msg.includes('rate limit') || msg.includes('too many requests')) {
        ...
        const seconds = Math.min(120 * sendAttempts.current, 600)
        sessionStorage.setItem('forgotpw_cooldown', String(Date.now() + seconds * 1000))
        setCooldown(seconds)
        setError(`Muitas tentativas. Aguarde ${seconds} segundos antes de solicitar um novo código.`)
      } else {
  ...
  async function handleResetPassword(e: FormEvent) {
    ...
    if (!token) {
      setError('Informe o código enviado por email.')
      return
    }
    setLoading(true)
      ...
      <div className="relative z-10 w-full max-w-md mx-4 -mt-24 animate-[slideUpFade_0.6s_ease-out]">
        ...
        <div className="bg-[#24283b]/70 backdrop-blur-xl border border-[#292e42] rounded-2xl shadow-2xl shadow-[#bb9af7]/5 p-8 md:p-10">
            ...
            <form onSubmit={handleResetPassword} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  maxLength={6}
                  required
                  className="w-full bg-[#1a1b26] border border-[#3b4261] rounded-lg text-[#c0caf5] text-base sm:text-sm px-4 py-3 placeholder-[#565f89] focus:outline-none focus:ring-2 focus:ring-[#bb9af7]/30 focus:border-[#bb9af7] transition-all tracking-[0.5em] text-center"
                />
              </div>
              <div>
                <label className="block text-xs uppercase tracking-wide text-[#a9b1d6] mb-1">
              ...
              <button
                ...
                className="w-full text-center text-sm text-[#a9b1d6] hover:text-[#c0caf5] transition-colors"
              >
                Voltar
              </button>
            </form>
          ...
          <button
            ...
            className="w-full mt-4 text-center text-sm text-[#a9b1d6] hover:text-[#c0caf5] transition-colors"
          >
            Voltar ao login
          </button>
        </div>
